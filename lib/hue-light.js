...
LightItem.prototype.parseColorRGB = function parseColorRGB(input, output) {
  ...
  if (_.isArray(input.xy) &&
    ...
    xyY[1] = limitValue(input.xy[0], 0, 1);
           ...
           _.isFinite(input.rgb[2]))
  {
    ...
    rgb[0] = limitValue(input.rgb[0], 0, 0xFF);
    rgb[1] = limitValue(input.rgb[1], 0, 0xFF);
    rgb[2] = limitValue(input.rgb[2], 0, 0xFF);
  ...
  output.bri = limitValue(Math.round(bri*(0xFF/100), 0, 0xFF));
  ...
  // Overwrite xy if we have hs support
  if (this.info.capability & LightCapability.HS) {
    output.colormode = 'hs';
